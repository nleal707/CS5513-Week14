...
// Import functions to get all snack IDs and the data for a specific snack
import { getAllPostIds, getPostData } from '../../lib/snacks-json';
...
// Export an async function to fetch data for a specific snack at build time
export async function getStaticProps({ params }) {
  // Fetch the data for a single transaction using the ID from the route parameters
  const snackData = await getPostData(params.id);
  // Return the fetched data as props to the snack component
  return {
    props: {
      snackData,
    },
...
export async function getStaticPaths() {
  // Get the array of all possible snack IDs
...
// Define and export the snack component, which displays a single snack
export default function Snack({ snackData }) {
      ...
      <article>
        ...
        <Container className={utilStyles.space}>
          <Row className={utilStyles.lightText}>
            ...
            <Col className="col-12">
              <p><strong>Snack Type:</strong> {snackData.snack_type || 'N/A'}</p>
            </Col>
          </Row>
        </Container>
      </article>
